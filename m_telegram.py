...
class ZuulMessengerPlugin:
	...
	def send_pin_code(self, update, context, query):
	...
	def menu_new_pin(self, update, context, query):
		...
		if self.access_manager.user_is_active(
			...
			if context.args:
				...
				self.send_pin_code(update, context, query)
	...
	def create_certificate(self, user_context, door_bot_name):
		"""Send acertificate."""
	...
	def menu_key_management(self, update, context, query):
	...
	def lend_key_callback(self, update, context, query):
		...
		user_context = UserContext.get_user_context(update, context, query)
		user_context.msg.reply_text(user_context._(
		...
		self.menu_key_management(update, context, query)
